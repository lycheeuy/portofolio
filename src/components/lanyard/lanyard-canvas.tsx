...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }: BandProps) {
  ...
  const rawTexture = useTexture(LANYARD_PNG);
  const texture = useMemo(() => {
    // Configuring a THREE.Texture in place is idiomatic; the object is mutable
    // by design. The immutability lint doesn't model THREE objects, so disable
    // it narrowly here.
  ...
  const cardMap = useMemo(
    () => materials.base.map as THREE.Texture,
    [materials.base.map],
  );
